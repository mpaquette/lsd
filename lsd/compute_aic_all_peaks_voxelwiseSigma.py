...
def main():
    ...
    # remove voxel from mask if any direction is sigma=0
    ...
    def _aic_loop(data):
        ...
        sigmas = data[N_dwi+4*N_dirs:2*N_dwi+4*N_dirs]
        ratio = data[2*N_dwi+4*N_dirs]
        ...
        loglikelihood = multigaussian_log_likelihood(diffs, sigmas)
    ...
    print('Concat data')
    concat_data = np.concatenate((data[..., ~gtab.b0s_mask], dirs, lens, sigma[..., ~gtab.b0s_mask], ratio*np.ones(dirs.shape[:3]+(1,))), axis=3)
